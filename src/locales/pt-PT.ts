...
const ptPT = {
  ...
  error: {
    ...
    workspace_required: "É necessário um contexto de workspace/organização ativo.",
    rate_limit_exceeded: "Demasiados pedidos. Por favor, tente novamente mais tarde.",
  },

  // ── conflict / optimistic concurrency ──────────────────────────────────
  conflict: {
    default: "O recurso foi modificado por outra pessoa. Recarregue e tente novamente.",
    song: "Esta música foi modificada por outra pessoa desde a última vez que a carregou.",
    folder: "Esta pasta foi modificada por outra pessoa desde a última vez que a carregou.",
    service: "Este culto foi modificado por outra pessoa desde a última vez que o carregou.",
  },
  ...
  notification: {
    ...
    folder_deleted_description: "{{songs}} música(s) e {{folders}} sub-pasta(s) foram removidas permanentemente.",
    backup_restored_title: "Backup restaurado — todos os dados substituídos",
    backup_restored_description: "Uma restauração completa importou {{folders}} pasta(s), {{songs}} música(s) e {{services}} culto(s).",
    settings_changed_title: "Definições da organização atualizadas",
    settings_changed_description: "Um administrador alterou uma ou mais definições do workspace.",
  },
} as const;

export type I18nKeys = typeof ptPT;
export default ptPT;
