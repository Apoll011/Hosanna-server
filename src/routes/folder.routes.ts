...
folderRouter.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    const result =
      action === 'delete_songs'
        ? await service.deleteWithSongs(req.params.id)
        : await service.deleteMovingSongsToRoot(req.params.id);
...
folderRouter.delete(
  ...
  asyncHandler(async (req, res) => {
    const service = new FolderService(req.db!);
    res.status(200).json(await service.deleteMovingSongsToRoot(req.params.id));
  }),
);
...
folderRouter.delete(
  ...
  asyncHandler(async (req, res) => {
    const service = new FolderService(req.db!);
    res.status(200).json(await service.deleteWithSongs(req.params.id));
  }),
);
